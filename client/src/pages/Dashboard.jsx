...
function Dashboard() {
  ...
  const unbalancedCount = devices.filter(d => d.metrics?.environmentStatus === 'unbalanced').length;
    ...
    <div>
      <div className="stats-row">
        ...
        <div className="stat-card purple">
          <div className="stat-label">调理中</div>
          <div className="stat-value">{unbalancedCount}<span className="stat-unit">台</span></div>
          <div className="stat-icon">⚠️</div>
        </div>
      ...
      <div className="device-grid">
        {devices.map(device => {
          ...
          return (
            <div
              key={device.id}
              className={`device-card ${isUnbalanced ? 'card-unbalanced' : ''}`}
            ...
            >
              <div className="device-card-header">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                  ...
                  {isUnbalanced && (
                    <span className="device-condition-warning">
                      ⚠️ 调理中
                    </span>
                  )}
                </div>
